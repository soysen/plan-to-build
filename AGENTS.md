...
> 每次收到新需求時，在呼叫任何工具（如 `view_file`, `list_dir`）或給出具體回覆前，**必須**先在對話輸出開頭寫下：
> `[Harness Gate Check: 進行中 / 需補充輸入 / 未啟動]`
> 
> **🚫 紅線行為 (RED LINES) 🚫**
> - **嚴禁**在 `.github/worklog/agent-status.md` 更新前，去讀取或分析專案中的業務程式碼（如 `.js`, `.ts` 等）。
> - **嚴禁**在使用者未確認「任務卡範圍」前，擅自開始除錯或規劃架構。
> - 若未通過第一與第二閘門，你的唯一合法動作是生成最小任務卡（Task Card）並要求補充。
