...
## 按需使用與 TDD 藍燈原則 (On-Demand & TDD Refactor Only)

> **[Guardrails] 避免過度重構**：
> 1. **非強制關卡 (Optional Step)**：本技能**絕非**每個開發切片都必須執行的 Mandatory 步驟。只要程式碼易讀、無重複、測試通過，即不需額外簡化。
> 2. **防範 Premature Abstraction (過度抽象)**：簡化是為了「消除重複與降低深度」，絕不能為了簡化而引入只用一次的 Class、Interface 或工廠模式。
> 3. **TDD 藍燈 (Refactor Phase) 按需觸發**：遵守 **Rule of Three（三原則）**，僅在出現三次明確重複，或函式超過 50 行、嵌套超過 3 層時使用。

## 適用時機

- TDD 藍燈 (Refactor) 階段發現明顯重複或複雜邏輯
- 函式超過 50 行或條件邏輯嵌套超過 3 層
- 有過度的抽象層（只被用到一次的 class/interface）

**不適用時機：**

- 程式碼已簡潔且測試通過（不需要「為了重構而重構」）
