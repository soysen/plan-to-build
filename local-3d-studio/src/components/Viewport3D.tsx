...
export const Viewport3D: React.FC<Viewport3DProps> = ({config, onSelectJoint, onJointRotate, onRendererReady}) => {
	const mountRef = useRef<HTMLDivElement>(null)
	const sceneRef = useRef<THREE.Scene | null>(null)
	const cameraRef = useRef<THREE.PerspectiveCamera | null>(null)
	const rendererRef = useRef<THREE.WebGLRenderer | null>(null)
	const currentGroupRef = useRef<THREE.Group | null>(null)
	const customModelCacheRef = useRef<THREE.Group | null>(null)
	const customModelUrlRef = useRef<string | null>(null)
	...
	useEffect(() => {
		...
		if (config.modelType === "humanoid_male") {
			...
			newGroup = buildQuadrupedMannequin(config.pose, config.renderMode, config.quadrupedSpecies, config.selectedJoint)
		...
		} else if (config.modelType === "custom_upload") {
			const modelUrl = config.reference.converted3DMeshUrl || config.reference.src
			if (modelUrl) {
				const proceedWithGroup = (gltfGroup: THREE.Group) => {
					if (currentGroupRef.current && sceneRef.current) {
						sceneRef.current.remove(currentGroupRef.current)
					}

					// Apply skeletal bone rotations in real time
					applyRotationsToSkeletalModel(gltfGroup, config.pose.joints)

					// Update rendering mode material and highlight selection
					applyRenderModeAndSelectionToCustomModel(gltfGroup, config.renderMode, config.selectedJoint)

					sceneRef.current?.add(gltfGroup)
					currentGroupRef.current = gltfGroup

					if (rendererRef.current && sceneRef.current && cameraRef.current && onRendererReady) {
						onRendererReady(rendererRef.current, sceneRef.current, cameraRef.current, gltfGroup)
					}
				}

				if (customModelUrlRef.current === modelUrl && customModelCacheRef.current) {
					proceedWithGroup(customModelCacheRef.current)
				} else {
					loadGLTFOrOBJModel(modelUrl, config.renderMode).then(gltfGroup => {
						customModelCacheRef.current = gltfGroup
						customModelUrlRef.current = modelUrl
						proceedWithGroup(gltfGroup)
					})
				}
				return
			}
	...
	const handleMouseDown = (e: React.MouseEvent) => {
		...
		if (e.button === 0) {
			if (config.selectedJoint && onSelectJoint) onSelectJoint(null)
			isDraggingRef.current = true
		}
	...
	return (
		<div className="relative w-full h-full overflow-hidden studio-glow select-none">
			...
			{config.reference.showOverlay && config.reference.src && (
				<div
					className="reference-corner-card pointer-events-none w-32 sm:w-40 md:w-48 rounded-xl border border-cyan-400/40 bg-slate-950/85 p-2 shadow-2xl shadow-black/50 backdrop-blur-md transition-opacity duration-300"
					style={{opacity: config.reference.overlayOpacity}}
				>
					<div className="mb-1 truncate text-[10px] font-semibold text-cyan-200">
						{config.reference.fileName || "Reference"}
					</div>
					<img
						src={config.reference.src}
						alt="Reference pose preview"
				...
				<div>
					...
					{config.selectedJoint && (
						<div className="text-[11px] text-emerald-400 mt-0.5 font-sans font-medium">
							🎯 當前已選中: <span className="font-bold capitalize">{config.selectedJoint}</span> (拖曳 3D
							旋轉環可即時擺姿)
						</div>
					)}
				</div>
